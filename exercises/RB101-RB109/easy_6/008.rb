...
# --------------------------------------------------------------------------- #
# initial (lengthy) solution:

def find_dup_initial(nums_arr)
  num_dump = []
  duplicate_num = nil
  nums_arr.each do |num|
    if num_dump.include?(num)
      duplicate_num = num
      break
    else
      num_dump << num
...
def find_dup(nums_arr)
  nums_arr.find { |num| nums_arr.count(num) == 2 }
end

# --------------------------------------------------------------------------- #
# EXAMPLES:
find_dup([1, 5, 3, 1]) == 1                                       # => true
find_dup([18,  9, 36, 96, 31, 19, 54, 75, 42, 15,
          ...
          7,  34, 57, 74, 45, 11, 88, 67,  5, 58]) == 73          # => true
